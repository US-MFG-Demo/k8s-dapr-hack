...
1. Create 3 Azure KeyVault secrets.
   ...
   az keyvault secret set --vault-name kv-dapr-demo --name smtp-username --value "_password"
   az keyvault secret set --vault-name kv-dapr-demo --name smtp-username --value "HX783-K2L7V-CRJ4A-5PN1G"
   ```

2. Run the following 3 commands to grant the managed identity access to your AKS cluster. Update the **CLIENT_ID** with the managed service identity client id, the **SUBSCRIPTION_ID** and the **NODE_RESOURCE_GROUP** is the **AKS Node Resource Group (auto-generated by AKS deployment, starts with MC_** not the default resource group your cluster resource and the other workshop resources are deployed to.
